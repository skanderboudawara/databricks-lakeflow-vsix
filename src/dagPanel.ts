...
import { JobDefinition, JobUpdates, Library, saveJobUpdates, JobParser } from './jobParser';
...
export class DagPanel {
  ...
  static createOrShow(extensionUri: vscode.Uri, job: JobDefinition, allJobNames: string[] = []): void {
    ...
    new DagPanel(panel, job, allJobNames);
  }

  private constructor(panel: vscode.WebviewPanel, job: JobDefinition, allJobNames: string[] = []) {
    this.panel = panel;
    DagPanel.panels.set(job.name, this);

    this.panel.webview.html = this.buildHtml(job, allJobNames);
    ...
    this.panel.webview.onDidReceiveMessage(
      async (message: { command: string; filePath?: string; notebookPath?: string; jobName?: string; updates?: JobUpdates }) => {
        ...
        if (message.command === 'saveJob' && message.filePath && message.jobName && message.updates) {
          try {
            ...
            this.panel.webview.postMessage({ type: 'saveError', error: err.message });
          }
        }

        // Immediate task add/remove — writes to file but does NOT reset pending edits
        if (message.command === 'applyJobChange' && message.filePath && message.jobName && message.updates) {
          try {
            saveJobUpdates(message.filePath, message.jobName, message.updates);
            const updatedJob = new JobParser().parseFile(message.filePath);
  ...
  private buildHtml(job: JobDefinition, allJobNames: string[] = []): string {
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .topbar-name:hover { border-color: var(--border); }
    .topbar-name:focus { border-color: var(--link); background: var(--hover); }
    ...
    .nt-section:last-child { border-bottom: none; }
    .nt-title {
      font-size: 12px;
      font-weight: 600;
      color: var(--fg-dim);
    ...
    .btn-add-confirm:hover { background: #1a4a8c; }
    .pill {
    ...
    .prop-input:focus { outline: none; border-color: var(--link); }
    select.prop-input { cursor: pointer; }
    .prop-input[type="number"] { width: 80px; }
    .prop-check-label {
      display: flex; align-items: center; gap: 7px; cursor: pointer; margin-top: 2px;
    }
    .prop-check-label .prop-label { margin: 0; }
    .topbar-btn.dirty { border-color: var(--orange); color: var(--orange); background: #2e1e00; }
    .topbar-btn.dirty:hover { background: #3e2e00; }
    .topbar-btn.saved { border-color: var(--green); color: var(--green); background: #163016; }
    ...
    .lib-modal-title { font-size: 13px; font-weight: 600; }
    .lib-modal-done {
      background: #163016; border: 1px solid var(--green);
      color: var(--green); border-radius: 4px;
      padding: 4px 14px; font-size: 11px; cursor: pointer;
    }
    .lib-modal-done:hover { background: #1e4a1e; }
    .lib-list { flex: 1; overflow-y: auto; min-height: 60px; }
    .lib-row {
    ...
    .lib-desc { font-size: 11px; flex: 1; word-break: break-all; font-family: var(--mono); line-height: 1.5; }
    .lib-desc .lib-sub { font-size: 10px; color: var(--fg-dim); margin-top: 1px; }
    .lib-remove-btn {
      background: none; border: none; color: var(--fg-dim);
      cursor: pointer; font-size: 16px; padding: 0 2px; flex-shrink: 0; line-height: 1;
    }
    .lib-remove-btn:hover { color: var(--red); }
    .lib-add-form {
    ...
    .lib-add-row { display: flex; gap: 8px; align-items: flex-end; }
    .lib-add-row label { font-size: 10px; color: var(--fg-dim); text-transform: uppercase; letter-spacing: 0.05em; display: block; margin-bottom: 2px; }
    .lib-add-row .prop-input { margin-top: 0; }
    .lib-add-btn {
      background: #1a3a6e; border: 1px solid var(--blue);
      color: var(--blue); border-radius: 4px;
      padding: 5px 14px; font-size: 11px; cursor: pointer; white-space: nowrap; flex-shrink: 0;
    }
    .lib-add-btn:hover { background: #2a4a7e; }
    .lib-serverless-note {
      margin: 8px 0 4px;
      padding: 8px 10px;
      background: #2e1e00;
      ...
      font-size: 11px; color: #f9a825; line-height: 1.5;
    }
    .lib-modify-btn {
    ...
    .lib-modify-btn:hover { background: var(--hover); }
    ...
    /* ── Key-value editor (spark conf, env vars) ───────────────────────────── */
    .kv-row {
      display: flex; align-items: center; gap: 4px;
      margin-bottom: 4px;
    }
    .kv-row .prop-input { margin-top: 0; }
    .kv-key { flex: 1; min-width: 0; }
    .kv-val { flex: 1.5; min-width: 0; }
    .kv-remove-btn {
      background: none; border: none; color: var(--fg-dim);
      cursor: pointer; font-size: 14px; padding: 0 2px; flex-shrink: 0; line-height: 1;
    }
    .kv-remove-btn:hover { color: var(--red); }
    .kv-add-row {
      display: flex; gap: 4px; margin-top: 2px;
    }
    .kv-add-row .prop-input { margin-top: 0; flex: 1; min-width: 0; }

    /* ── Table name / dep list items ────────────────────────────────────────── */
    .list-item-row {
      display: flex; align-items: center; gap: 6px;
      padding: 3px 0; border-bottom: 1px solid var(--border);
    }
    .list-item-row:last-of-type { border-bottom: none; }
    .list-item-val { flex: 1; font-family: var(--mono); font-size: 10px; word-break: break-all; }
    .list-item-btn {
      background: none; border: none; color: var(--fg-dim);
      cursor: pointer; font-size: 14px; padding: 0 2px; flex-shrink: 0; line-height: 1;
    }
    .list-item-btn:hover { color: var(--red); }
    .list-add-row {
      display: flex; gap: 4px; margin-top: 6px;
    }
    .list-add-row .prop-input { margin-top: 0; flex: 1; }
    .section-add-btn {
    ...
    .section-add-btn:hover { border-color: var(--blue); color: var(--blue); }
    .section-danger-btn {
      background: transparent; border: 1px solid var(--border);
      color: var(--red); border-radius: 4px;
      padding: 3px 10px; font-size: 10px; cursor: pointer; margin-left: 6px;
    }
    .section-danger-btn:hover { background: #3a1a1a; }
    .add-cluster-form, .add-env-form {
      background: var(--bg);
      border: 1px solid var(--border);
      border-radius: 4px;
      padding: 10px;
    ...
    .add-cluster-form.visible, .add-env-form.visible { display: flex; }
    .add-form-btns { display: flex; gap: 6px; margin-top: 4px; }
    .add-form-btns button { flex: 1; padding: 4px; font-size: 11px; border-radius: 3px; cursor: pointer; }
    .add-form-confirm { background: #163016; border: 1px solid var(--green); color: var(--green); }
    .add-form-confirm:hover { background: #1e4a1e; }
    .add-form-cancel { background: transparent; border: 1px solid var(--border); color: var(--fg-dim); }
    .add-form-cancel:hover { background: var(--hover); }

    /* ── Port circles ───────────────────────────────────────────────────────── */
    .port-grp { opacity: 0; transition: opacity 0.12s; }
    .port-grp.port-in { cursor: default; }
    .port-grp.port-out { cursor: crosshair; }
    /* Show ports when hovering the nearest task node */
    svg#dag:hover .port-grp { opacity: 0.55; }
    svg#dag:hover .port-grp:hover { opacity: 1; }

    /* ── Edge interactive groups ────────────────────────────────────────────── */
    .edge-del-btn { pointer-events: none; opacity: 0; transition: opacity 0.1s; }
    .edge-grp:hover .edge-del-btn { opacity: 1; pointer-events: all; }

    /* ── Resize handle (between left sidebar and canvas) ───────────────────── */
    .resize-handle {
      width: 4px;
      flex-shrink: 0;
      cursor: col-resize;
    ...
    /* Sidebar toggle button (on the right edge of canvas when sidebar collapsed) */
    .sidebar-toggle-fab {
      position: absolute;
      right: 0;
      top: 50%;
    ...
    .bottom-bar-resize:hover,
    .bottom-bar-resize.dragging {
      background: var(--vscode-focusBorder, #007acc);
    }
    .bottom-bar-inner {
      flex: 1;
...
<div class="body">
  ...
  <!-- Right sidebar (job info only) -->
  <div class="sidebar" id="sidebar">
    <div class="sidebar-inner" id="sidebar-inner">

      <!-- Strip: label + collapse -->
      ...
      <!-- Job panel content (always shown) -->
...
(function () {
  ...
  let job = ${jobJson};
  const allJobNames = ${allJobNamesJson};
  ...
  function getEffectiveDepsFor(taskKey) {
    if (taskKey in taskDepsOverride) return taskDepsOverride[taskKey];
  ...
  function getPortPos(taskKey, portType) {
    ...
    if (found.type === 'condition') {
      ...
      if (portType === 'in')        return { x: p.x,       y: cy };
      if (portType === 'out-true')  return { x: p.x + CS*2, y: cy };
      if (portType === 'out-false') return { x: cx,         y: p.y + CS*2 };
    } else {
      if (portType === 'in')  return { x: p.x,      y: p.y + NH/2 };
      if (portType === 'out') return { x: p.x + NW,  y: p.y + NH/2 };
    }
    return null;
  }
  ...
  function redrawEdges() {
    ...
    const tmap2 = {};
    for (const t of vtasks) tmap2[t.task_key] = t;
  ...
  function draw(tasks, pos) {
    ...
    for (const t of tasks) {
      ...
      g.addEventListener('mousedown', function(e) {
        if (e.target.closest('.port-grp')) return;
        e.stopPropagation();
      ...
      g.addEventListener('click', function(e) {
        e.stopPropagation();
        if (taskDragWasMoved) { taskDragWasMoved = false; return; }
  ...
  function deselect() {
    if (selectedKey) {
      ...
      if (n) {
        ...
        if (shape) { shape.setAttribute('stroke-width','1.5'); shape.setAttribute('stroke','#d4d4d4'); }
      }
    }
    selectedKey = null;
    closeBottomBar();
    if (sidebarMode !== 'job') {
      ...
      if (breadcrumb) breadcrumb.textContent = 'Job';
    }
    showJobPanel();
  }
  ...
  function selectTask(key) {
    ...
    if (n) {
      const shape = n.querySelector('rect') || n.querySelector('polygon');
      if (shape) { shape.setAttribute('stroke-width','3'); shape.setAttribute('stroke','#4a9eff'); }
    }
  ...
  window.markDirty = function(input) {
    ...
    else if (input.type === 'number') val = input.value !== '' ? Number(input.value) : null;
  ...
  function markClean(savedOk) {
    ...
    if (savedOk) {
      ...
      setTimeout(() => { if (!isDirty) { saveBtn.textContent = 'Save'; saveBtn.classList.remove('saved'); } }, 2000);
    } else {
      saveBtn.textContent = 'Save';
    }
  }
  ...
  revertBtn.addEventListener('click', function() {
    ...
    for (const k of Object.keys(pendingUpdates)) delete pendingUpdates[k];
    for (const k of Object.keys(taskDepsOverride)) delete taskDepsOverride[k];
    pendingNewClusters = [];
    ...
    document.getElementById('hdr-tasks').textContent =
      job.tasks.length + ' task' + (job.tasks.length !== 1 ? 's' : '');
    requestAnimationFrame(function() { if (_bbox) fitView(_bbox); });
    selectedKey = null;
    closeBottomBar();
    showJobPanel();
  });
  ...
  window.addEventListener('message', evt => {
    ...
    if (msg.type === 'saveError') {
      saveBtn.textContent = 'Save'; saveBtn.classList.remove('dirty','saved'); isDirty = true;
    }
    ...
    if (msg.type === 'jobChanged' && msg.updatedJob) {
      ...
      for (const k of Object.keys(taskDepsOverride)) {
        if (!validKeys.has(k)) { delete taskDepsOverride[k]; }
      }
      ...
      for (const k of Object.keys(pendingUpdates)) {
        ...
        if (taskKey && !validKeys.has(taskKey)) { delete pendingUpdates[k]; }
      }
  ...
  function setPath(obj, keys, value) {
    ...
    for (let i = 0; i < keys.length - 1; i++) {
      const k = keys[i];
      if (cur[k] === undefined || cur[k] === null) cur[k] = isNaN(Number(keys[i+1])) ? {} : [];
      cur = cur[k];
    }
  ...
  function libDesc(lib) {
    if (lib.type === 'whl' || lib.type === 'jar' || lib.type === 'requirements') return lib.path || '';
    if (lib.type === 'pypi') return lib.package + (lib.repo ? \`<div class="lib-sub">repo: \${esc(lib.repo)}</div>\` : '');
    if (lib.type === 'maven') {
      let d = lib.coordinates || '';
      if (lib.repo) d += \`<div class="lib-sub">repo: \${esc(lib.repo)}</div>\`;
      if (lib.exclusions?.length) d += \`<div class="lib-sub">excl: \${esc(lib.exclusions.join(', '))}</div>\`;
      return d;
    }
  ...
  window.addLib = function() {
    ...
    if (type === 'whl' || type === 'jar' || type === 'requirements') {
      ...
      if (!path) { document.getElementById('lib-path').focus(); return; }
      lib = { type, path };
      document.getElementById('lib-path').value = '';
    } else if (type === 'pypi') {
      const pkg = document.getElementById('lib-pypi-pkg').value.trim();
      if (!pkg) { document.getElementById('lib-pypi-pkg').focus(); return; }
      ...
      if (!coords) { document.getElementById('lib-maven-coords').focus(); return; }
      const repo = document.getElementById('lib-repo').value.trim();
    ...
    if (lib) { modalLibs.push(lib); renderModalList(); }
  };

  window.openLibraryModal = function(taskKey) {
    const backdrop = document.getElementById('lib-backdrop');
    if (!backdrop) { console.error('lib-backdrop not found'); return; }
  ...
  document.getElementById('lib-backdrop').addEventListener('click', (e) => {
    if (e.target === document.getElementById('lib-backdrop')) closeAndSaveModal();
  });
  ...
  function openClusterEditor(key) {
    ...
    if (Number(_numW) === 0) clusterEditorWorkerMode = 'single_node';
    else if (_minW != null || _maxW != null) clusterEditorWorkerMode = 'autoscale';
    else clusterEditorWorkerMode = 'fixed';
  ...
  function _saveClusterEditorKVToPending() {
    if (!clusterEditorKey) return;
    const scObj = {};
    clusterEditorSparkConf.forEach(function(row) { if (row.k.trim()) scObj[row.k.trim()] = row.v; });
    pendingUpdates[\`jobClusters.\${clusterEditorKey}.sparkConf\`] = scObj;
    const seObj = {};
    clusterEditorSparkEnvVars.forEach(function(row) { if (row.k.trim()) seObj[row.k.trim()] = row.v; });
  ...
  function renderClusterEditorContent() {
    ...
    panelContent.innerHTML = \`
      ...
      <div style="padding:8px 14px 14px">
        ...
        <hr style="border:none;border-top:1px solid var(--border);margin:10px 0">
        <div class="prop-label" style="margin-bottom:6px">Spark Config
          <span id="btn-add-sc" style="cursor:pointer;color:var(--link);font-size:10px;margin-left:6px">+ Add</span>
        </div>
        <div id="sc-rows">\${scRows}</div>
        <hr style="border:none;border-top:1px solid var(--border);margin:10px 0">
        <div class="prop-label" style="margin-bottom:6px">Spark Env Vars
          <span id="btn-add-se" style="cursor:pointer;color:var(--link);font-size:10px;margin-left:6px">+ Add</span>
        </div>
        <div id="se-rows">\${seRows}</div>
        <hr style="border:none;border-top:1px solid var(--border);margin:10px 0">
  ...
  function libEdAdd() {
    ...
    if (type === 'whl' || type === 'jar' || type === 'requirements') {
      ...
      if (!coords) { coordsEl.focus(); return; }
      const repo = repoEl.value.trim();
      const excl = exclEl.value.trim() ? exclEl.value.trim().split('\\n').map(function(s) { return s.trim(); }).filter(Boolean) : [];
      lib = Object.assign({ type: 'maven', coordinates: coords },
        repo ? { repo } : {},
        excl.length ? { exclusions: excl } : {});
      coordsEl.value = '';
      repoEl.value = '';
      exclEl.value = '';
    }
    if (lib) { libEditorLibs.push(lib); renderLibEditorContent(); }
  }
  ...
  function propSelect(label, value, fieldPath, options) {
    const opts = options.map(o =>
      \`<option value="\${esc(o.v)}"\${(o.v === (value || '')) ? ' selected' : ''}>\${esc(o.l)}</option>\`
    ).join('');
    return \`<div class="prop">
      <div class="prop-label">\${esc(label)}</div>
  ...
  function showJobPanel() {
    ...
    if (job.parameters.length > 0) {
      paramBody = job.parameters.map((p, i) =>
    ...
    // --- Queue & concurrency ---
    const queueBody =
      propEdit('Queue enabled', getVal('queueEnabled', job.queueEnabled), 'queueEnabled', 'checkbox') +
    ...
    panelContent.addEventListener('click', function(e) {
      ...
      if (editBtn) { openClusterEditor(editBtn.dataset.editCluster); return; }
    ...
    if (showAddClusterBtn) {
      showAddClusterBtn.addEventListener('click', function() {
        ...
        if (form) { form.classList.toggle('visible'); showAddClusterBtn.style.display = form.classList.contains('visible') ? 'none' : ''; }
      });
    }
    ...
    if (confirmAddClusterBtn) {
      confirmAddClusterBtn.addEventListener('click', function() {
        ...
        if (!keyVal) { if(inputs[0]) inputs[0].focus(); return; }
    ...
    if (showAddEnvBtn) {
      showAddEnvBtn.addEventListener('click', function() {
        ...
        if (form) { form.classList.toggle('visible'); showAddEnvBtn.style.display = form.classList.contains('visible') ? 'none' : ''; }
      });
    }
  ...
  function showTaskPanel(task) {
    ...
    <div class="section-body" style="border-top:1px solid var(--border);padding-top:8px">\${
      propSelect('Run if', getVal(\`tasks.\${tk}.run_if\`, task.run_if||''), \`tasks.\${tk}.run_if\`, [
        {v:'',l:'(default)'},
    ...
    let computeBody =
      propSelect('Job cluster',   getVal(\`tasks.\${tk}.job_cluster_key\`,  task.job_cluster_key||''),  \`tasks.\${tk}.job_cluster_key\`,  clusterOpts) +
    ...
    if (effectiveEnvKey) {
      ...
      const libItems = currentLibs.length > 0
        ? currentLibs.map(lib => {
            const color = LIB_COLORS[lib.type] || '#888';
            let desc = '';
            if (lib.type === 'whl' || lib.type === 'jar' || lib.type === 'requirements') desc = lib.path || '';
            else if (lib.type === 'pypi') desc = (lib.package || '') + (lib.repo ? \` (repo: \${lib.repo})\` : '');
            else if (lib.type === 'maven') desc = (lib.coordinates || '') + (lib.repo ? \` (repo: \${lib.repo})\` : '');
    ...
    // --- Remove task button ---
    parts.push(\`<div style="padding:10px 14px 14px">
      <button id="btn-remove-task"
        style="background:transparent;border:1px solid var(--red);color:var(--red);border-radius:4px;padding:4px 14px;font-size:11px;cursor:pointer;width:100%"
    ...
    if (libModifyBtn) libModifyBtn.addEventListener('click', () => openLibraryEditor(tk));
  ...
  window.addDepToTask = function(taskKey, fromKey, outcome) {
    if (!fromKey) return;
    var deps = getEffectiveDepsFor(taskKey).slice();
    if (deps.some(function(d) { return d.task_key === fromKey; })) return;
    var newDep = { task_key: fromKey };
    if (outcome) newDep.outcome = outcome;
    deps.push(newDep);
    taskDepsOverride[taskKey] = deps;
    pendingUpdates['tasks.' + taskKey + '.depends_on'] = deps;
    markDirtyDirect();
  ...
  function showNewTaskPanel() {
    ...
    document.getElementById('btn-nt-confirm').addEventListener('click', function() {
      ...
      if (!key) { document.getElementById('nt-key').focus(); return; }
  ...
  window.addEventListener('mouseup', (e) => {
    ...
    if (draggingTask) {
      if (draggingTask.moved) {
        drawPorts(); redrawEdges();
        dagSvg.style.cursor = '';
      }
      draggingTask = null;
      return;
    }
    drag=false; dagSvg.classList.remove('panning');
  });
